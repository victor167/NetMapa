var loginActivity =
{
  ...
  load: function(){
	...
	$$('#contentLogin #btnIngresarLogin').on('click',loginActivity.sendLogin);
  	...
  	//myApp.onPageInit('masterlogin', function (page) {
	        		...
	        		if(typeof respondBody.success !=='undefined' && respondBody.success)
	        		{
	        			if(typeof respondBody.data.login !=='undefined' && respondBody.data.login)
	        			{
							...
							if(typeof token !== 'undefined')
							{
								...
								/*DB.query("UPDATE session SET value='1' WHERE name='connect'",function(){
									DB.query("UPDATE session SET value='" + token + "' WHERE name='token'",function(){
										DB.query("UPDATE session SET value='" + username + "' WHERE name='user'",function(){
											DB.query("UPDATE session SET value='" + namePerson + "' WHERE name='namePerson'",function(){
												DB.query("UPDATE session SET value='" + profile + "' WHERE name='profile'",function(){
													loginActivity.loginSuccess();
												});
											});
										});
